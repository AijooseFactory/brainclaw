...
import {
  listAgentBrainClawMemories,
  syncAgentBrainClawMemoryBackup,
  updateAgentBrainClawMemory,
} from "../brainclaw-memory.js";
import {
  ErrorCodes,
  errorShape,
  formatValidationErrors,
  validateAgentsBrainClawMemoryListParams,
  validateAgentsBrainClawMemoryUpdateParams,
...
export const agentsHandlers: GatewayRequestHandlers = {
  ...
  "agents.files.set": async ({ params, respond }) => {
    ...
    const shouldSyncBrainClawBackup = MEMORY_FILE_NAMES.includes(name as (typeof MEMORY_FILE_NAMES)[number]);
    let previousContent: string | null = null;
    if (shouldSyncBrainClawBackup && resolvedPath.kind === "ready") {
      try {
        const safeRead = await readLocalFileSafely({ filePath: resolvedPath.ioPath });
        previousContent = safeRead.buffer.toString("utf-8");
      } catch (error) {
    ...
    if (shouldSyncBrainClawBackup) {
      try {
        await syncAgentBrainClawMemoryBackup({
          agentId,
          content,
          filePath,
        });
        ...
        const isBrainClawUnavailable =
          message.includes("BrainClaw is not installed") ||
          message.includes("BrainClaw is installed but disabled");
        if (isBrainClawUnavailable) {
        ...
        if (previousContent !== null) {
          try {
            ...
            // Best effort rollback only; the sync error is still the primary failure.
          }
        }
        respond(
          false,
          ...
          errorShape(
            ...
            `failed to synchronize BrainClaw memory backup: ${message}`,
          ),
        );
        return;
      }
  ...
  "agents.memory.list": async ({ params, respond }) => {
    if (!validateAgentsBrainClawMemoryListParams(params)) {
      respondInvalidMethodParams(
        respond,
        "agents.memory.list",
        validateAgentsBrainClawMemoryListParams.errors,
      );
      return;
    }
    ...
    const agentId = resolveAgentIdOrError(String(params.agentId ?? ""), cfg);
    if (!agentId) {
      respondAgentNotFound(respond, String(params.agentId ?? ""));
      return;
    }

    try {
      const result = await listAgentBrainClawMemories({
        agentId,
        ...
        includeSuperseded:
          typeof params.includeSuperseded === "boolean" ? params.includeSuperseded : false,
      });
      respond(true, result, undefined);
    } catch (error) {
      respond(
        ...
        errorShape(
          ErrorCodes.UNAVAILABLE,
          `failed to load BrainClaw memory: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
    }
  },
  "agents.memory.update": async ({ params, respond }) => {
    if (!validateAgentsBrainClawMemoryUpdateParams(params)) {
      respondInvalidMethodParams(
        respond,
        "agents.memory.update",
        validateAgentsBrainClawMemoryUpdateParams.errors,
      );
      return;
    }
    ...
    try {
      const result = await updateAgentBrainClawMemory({
        agentId,
      ...
      respond(
        ...
        errorShape(
          ErrorCodes.UNAVAILABLE,
          `failed to update BrainClaw memory: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
    }
  },
