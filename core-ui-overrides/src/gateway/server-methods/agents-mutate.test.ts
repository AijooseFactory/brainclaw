...
const mocks = vi.hoisted(() => ({
  ...
  listAgentBrainClawMemories: vi.fn(async (params: { agentId: string }) => ({
  ...
  updateAgentBrainClawMemory: vi.fn(async (params: { agentId: string; memoryId: string }) => ({
    ok: true,
    agentId: params.agentId,
    previousMemoryId: params.memoryId,
    memory: {
      id: "mem-2",
      content: "Updated BrainClaw memory content.",
      metadata: {
        memory_class: "decision",
        memory_type: "technical",
        status: "active",
  ...
  syncAgentBrainClawMemoryBackup: vi.fn(
    async (params: { agentId: string; content: string; filePath: string }) => ({
      ok: true,
      agentId: params.agentId,
      filePath: params.filePath,
      contentHash: "test-hash",
      mirroredEntryCount: 1,
    }),
  ),
}));
...
vi.mock("../brainclaw-memory.js", () => ({
  listAgentBrainClawMemories: mocks.listAgentBrainClawMemories,
  updateAgentBrainClawMemory: mocks.updateAgentBrainClawMemory,
  syncAgentBrainClawMemoryBackup: mocks.syncAgentBrainClawMemoryBackup,
}));
...
describe("agents.memory.list", () => {
  it("returns BrainClaw memory entries for the selected agent", async () => {
    const { respond, promise } = makeCall("agents.memory.list" as keyof typeof agentsHandlers, {
      agentId: "main",
      query: "rollout",
      area: "all",
      limit: 25,
    ...
    expect(mocks.listAgentBrainClawMemories).toHaveBeenCalledWith({
      agentId: "main",
      query: "rollout",
      area: "all",
      limit: 25,
...
describe("agents.memory.update", () => {
  it("supersedes BrainClaw memory content for the selected agent", async () => {
    const { respond, promise } = makeCall("agents.memory.update" as keyof typeof agentsHandlers, {
      agentId: "main",
      memoryId: "mem-1",
      content: "Updated BrainClaw memory content.",
    ...
    expect(mocks.updateAgentBrainClawMemory).toHaveBeenCalledWith({
      agentId: "main",
      memoryId: "mem-1",
      content: "Updated BrainClaw memory content.",
      reason: "Control UI correction",
...
describe("agents.files.set", () => {
  ...
  it("syncs MEMORY.md edits into the BrainClaw backup mirror", async () => {
    const { respond, promise } = makeCall("agents.files.set", {
      agentId: "main",
      name: "MEMORY.md",
      content: "# Main Memory\n\nUpdated memory mirror.\n",
    });
    await promise;

    expect(mocks.writeFileWithinRoot).toHaveBeenCalled();
    expect(mocks.syncAgentBrainClawMemoryBackup).toHaveBeenCalledWith({
      agentId: "main",
      content: "# Main Memory\n\nUpdated memory mirror.\n",
      filePath: "/workspace/test-agent/MEMORY.md",
    });
    expect(respond).toHaveBeenCalledWith(
      true,
      expect.objectContaining({
