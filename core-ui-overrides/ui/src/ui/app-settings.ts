...
import { loadAgentBrainClawMemories } from "./controllers/agent-memory.ts";
...
type SettingsHost = {
  ...
  agentsPanel?: "overview" | "memory" | "files" | "tools" | "skills" | "channels" | "cron";
  agentMemoryView?: "file" | "brainclaw";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "agents") {
    ...
    if (agentId) {
      ...
      if (host.agentsPanel === "memory") {
        if (host.agentMemoryView === "brainclaw") {
          void loadAgentBrainClawMemories(host as unknown as OpenClawApp, agentId);
        } else {
          void loadAgentMemoryFile(host as unknown as OpenClawApp, agentId);
        }
      }
...
async function loadAgentMemoryFile(host: OpenClawApp, agentId: string) {
  await loadAgentFiles(host, agentId);
  const files = host.agentFilesList?.agentId === agentId ? host.agentFilesList.files : [];
  const memoryFile =
    files.find((file) => file.name === DEFAULT_AGENT_MEMORY_FILENAME) ??
    files.find((file) => file.name === DEFAULT_AGENT_MEMORY_ALT_FILENAME);
  const fileName = memoryFile?.name ?? DEFAULT_AGENT_MEMORY_FILENAME;
  host.agentFileActive = fileName;
  if (memoryFile?.missing === true || !memoryFile) {
    if (!Object.hasOwn(host.agentFileContents, fileName)) {
      host.agentFileContents = { ...host.agentFileContents, [fileName]: "" };
    }
    if (!Object.hasOwn(host.agentFileDrafts, fileName)) {
      host.agentFileDrafts = { ...host.agentFileDrafts, [fileName]: "" };
    }
    return;
  }
  await loadAgentFileContent(host, agentId, fileName);
}
