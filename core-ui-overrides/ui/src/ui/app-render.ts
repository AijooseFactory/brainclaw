...
import {
  loadAgentBrainClawMemories,
  saveAgentBrainClawMemory,
  selectAgentBrainClawMemory,
...
export function renderApp(state: AppViewState) {
  ...
  const resetAgentBrainClawMemoryState = () => {
    state.agentBrainClawMemoryList = null;
    state.agentBrainClawMemoryError = null;
    state.agentBrainClawMemoryLoading = false;
    state.agentBrainClawMemorySelectedId = null;
    state.agentBrainClawMemoryDraft = "";
    state.agentBrainClawMemorySaving = false;
  };
  const resolveLoadedAgentFiles = (agentId: string) =>
    state.agentFilesList?.agentId === agentId ? state.agentFilesList : null;
  const resolveAgentMemorySelection = (agentId: string) => {
    const loaded = resolveLoadedAgentFiles(agentId);
    const files = loaded?.files ?? [];
    const entry =
      files.find((file) => file.name === DEFAULT_AGENT_MEMORY_FILENAME) ??
      files.find((file) => file.name === DEFAULT_AGENT_MEMORY_ALT_FILENAME);
    if (entry) {
      return {
        name: entry.name,
  ...
  const seedMissingAgentFile = (name: string) => {
    if (!Object.hasOwn(state.agentFileContents, name)) {
      state.agentFileContents = { ...state.agentFileContents, [name]: "" };
    }
    if (!Object.hasOwn(state.agentFileDrafts, name)) {
      state.agentFileDrafts = { ...state.agentFileDrafts, [name]: "" };
    }
  };
  const loadAgentMemoryPanel = async (agentId: string, options?: { reload?: boolean }) => {
    if (state.agentMemoryView === "brainclaw") {
      await loadAgentBrainClawMemories(state, agentId, { page: 1 });
      return;
    }
    if (options?.reload || resolveLoadedAgentFiles(agentId) == null) {
      await loadAgentFiles(state, agentId);
    }
    const memoryFile = resolveAgentMemorySelection(agentId);
    state.agentFileActive = memoryFile.name;
    if (memoryFile.missing) {
      seedMissingAgentFile(memoryFile.name);
      return;
    }
    await loadAgentFileContent(state, agentId, memoryFile.name);
  };
          ...
          state.tab === "agents"
            ? lazyRender(lazyAgents, (m) =>
                m.renderAgents({
                  ...
                  agentMemory: {
                    view: state.agentMemoryView,
                    listLoading: state.agentBrainClawMemoryLoading,
                    listError: state.agentBrainClawMemoryError,
                    list: state.agentBrainClawMemoryList,
                    selectedId: state.agentBrainClawMemorySelectedId,
                    draft: state.agentBrainClawMemoryDraft,
                    saving: state.agentBrainClawMemorySaving,
                    query: state.agentBrainClawMemoryQuery,
                    memoryClass: state.agentBrainClawMemoryClass,
                  ...
                  onRefresh: async () => {
                    ...
                    if (state.agentsPanel === "memory" && refreshedAgentId) {
                      if (state.agentMemoryView === "brainclaw") {
                        void loadAgentBrainClawMemories(state, refreshedAgentId, { page: 1 });
                      } else {
                        void loadAgentMemoryPanel(refreshedAgentId, { reload: true });
                      }
                    }
                  ...
                  onSelectAgent: (agentId) => {
                    ...
                    resetAgentBrainClawMemoryState();
                    ...
                    if (state.agentsPanel === "memory") {
                      if (state.agentMemoryView === "brainclaw") {
                        void loadAgentBrainClawMemories(state, agentId, { page: 1 });
                      } else {
                        void loadAgentMemoryPanel(agentId);
                      }
                    }
                  ...
                  onSelectPanel: (panel) => {
                    ...
                    if ((panel === "files" || panel === "memory") && resolvedAgentId) {
                      if (state.agentFilesList?.agentId !== resolvedAgentId) {
                        resetAgentFilesState();
                      }
                      if (
                        panel === "memory" &&
                        state.agentBrainClawMemoryList?.agentId !== resolvedAgentId
                      ) {
                        resetAgentBrainClawMemoryState();
                      }
                      if (panel === "files") {
                      ...
                      if (panel === "memory") {
                        if (state.agentMemoryView === "brainclaw") {
                          void loadAgentBrainClawMemories(state, resolvedAgentId, { page: 1 });
                        } else {
                          void loadAgentMemoryPanel(resolvedAgentId);
                        }
                      }
                    }
                    if (panel === "skills") {
                  ...
                  onLoadFiles: (agentId) => {
                    if (state.agentsPanel === "memory") {
                      if (state.agentMemoryView === "brainclaw") {
                        ...
                        void loadAgentMemoryPanel(agentId, { reload: true });
                      }
                      return;
                    }
                    void loadAgentFiles(state, agentId);
                  ...
                  onSelectFile: (name) => {
                    ...
                    const loadedFiles = resolveLoadedAgentFiles(resolvedAgentId);
                    const selectedFile =
                      loadedFiles?.files.find((file) => file.name === name) ?? null;
                    if (
                      selectedFile?.missing ||
                      ((name === DEFAULT_AGENT_MEMORY_FILENAME ||
                        name === DEFAULT_AGENT_MEMORY_ALT_FILENAME) &&
                        state.agentsPanel === "memory" &&
                        !selectedFile)
                    ) {
                      seedMissingAgentFile(name);
                      return;
                    }
                  ...
                  onSelectMemoryView: (view) => {
                    state.agentMemoryView = view;
                    if (!resolvedAgentId || state.agentsPanel !== "memory") {
                      return;
                    }
                    if (view === "brainclaw") {
                      if (state.agentBrainClawMemoryList?.agentId !== resolvedAgentId) {
                        resetAgentBrainClawMemoryState();
                      }
                      void loadAgentBrainClawMemories(state, resolvedAgentId, { page: 1 });
                      return;
                    ...
                    void loadAgentMemoryPanel(resolvedAgentId, { reload: true });
                  },
                  onBrainClawMemoryRefresh: (page) => {
                    if (!resolvedAgentId) {
                      return;
                    }
                    void loadAgentBrainClawMemories(state, resolvedAgentId, { page: page ?? 1 });
                  },
                  onBrainClawMemorySelect: (memoryId) => {
                    selectAgentBrainClawMemory(state, memoryId);
                  },
                  onBrainClawMemoryDraftChange: (content) => {
                    state.agentBrainClawMemoryDraft = content;
                  },
                  onBrainClawMemoryReset: () => {
                    const selected =
                      state.agentBrainClawMemoryList?.items.find(
                        (item) => item.id === state.agentBrainClawMemorySelectedId,
                      ) ?? null;
                    state.agentBrainClawMemoryDraft = selected?.content ?? "";
                  },
                  onBrainClawMemorySave: () => {
                    if (!resolvedAgentId || !state.agentBrainClawMemorySelectedId) {
                      return;
                    }
                    void saveAgentBrainClawMemory(
                      state,
                      resolvedAgentId,
                      state.agentBrainClawMemorySelectedId,
                      state.agentBrainClawMemoryDraft,
                    );
                  },
                  onBrainClawMemoryQueryChange: (next) => {
                    state.agentBrainClawMemoryQuery = next;
                  },
                  onBrainClawMemoryClassChange: (next) => {
                    state.agentBrainClawMemoryClass = next;
                  },
                  onBrainClawMemoryLimitChange: (next) => {
                    state.agentBrainClawMemoryLimit = next;
                  },
                  onBrainClawMemoryIncludeSupersededChange: (next) => {
                    state.agentBrainClawMemoryIncludeSuperseded = next;
                  },
