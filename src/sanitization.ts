...
let cachedSensitiveFields: string[] | null = null;

function getSensitiveFields(): string[] {
  if (cachedSensitiveFields) return cachedSensitiveFields;
  ...
  try {
    ...
    if (fs.existsSync(manifestPath)) {
      ...
      ['postgresUrl', 'weaviateUrl', 'neo4jUrl'].forEach(u => {
        if (!fields.includes(u)) fields.push(u);
      });
 ...
 */
export function redactConfigForLogs(config: any): any {
